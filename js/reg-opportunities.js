 ...
 * Ecological Registry — Opportunity Engine · Gardener & Son v1.0
 *
 ...
 * 4. Monetisable. Every opportunity carries delivery (who does it), effort and
 *    cost band — the hooks for Plants of Place packs, G&S installs, Tend and
 *    paid verification. Public surfaces can hide cost; internal ones show it.
 *
...
(function (root) {
  ...
  var DELIVERY_LABEL = {
    diy: 'Do it yourself', plants_of_place: 'Plants of Place', gs_install: 'G&S install',
    tend: 'Tend', verification: 'G&S verification'
  };

  // Two families of action. `ecological` = the garden actually changes.
  // `record` = evidence / verification: the record strengthens, ecology doesn't.
  // Public surfaces lead with ecological so nothing reads as pay-to-score.
  function groupOf(pillar, delivery) {
    return (pillar === 'evidence' || delivery === 'verification') ? 'record' : 'ecological';
  }
  ...
  function catalogue(record) {
    ...
    if (spNext !== null && (b.indigenous_species_current || 0) < speciesTarget) {
      ...
      add({
        ...
        effort: 'moderate', cost: '$$', delivery: 'plants_of_place',
    ...
    if (!b.indigenous_dominant) {
      add({
        ...
        effort: 'project', cost: '$$', delivery: 'gs_install',
    ...
    if (caNext !== null && caNext <= canopyTarget + 5) {
      add({
        ...
        effort: 'project', cost: '$$$', delivery: 'gs_install', season: 'Autumn–winter planting',
    ...
    if ((sw.soil_health_score || 0) < (sw.soil_health_max || 5)) {
      add({
        ...
        effort: 'moderate', cost: '$$', delivery: 'tend',
    ...
    if ((sw.water_function_score || 0) < (sw.water_function_max || 5)) {
      add({
        ...
        effort: 'moderate', cost: '$$', delivery: 'gs_install',
        mutate: function (r) { r.soil_water.water_function_score = (sw.water_function_score || 0) + 1; }
      });
    }
    if (!sw.has_moisture_basin) add({ id: 'moisture_basin', pillar: 'soil_water', label: 'Install a seasonal moisture basin', detail: 'A shallow basin that holds winter water and supports frogs and insects.', effort: 'moderate', cost: '$$', delivery: 'gs_install', mutate: function (r) { r.soil_water.has_moisture_basin = true; } });
    if (!sw.has_rainwater_system) add({ id: 'rainwater', pillar: 'soil_water', label: 'Add rainwater capture', detail: 'Tank or redirect to keep the garden watered through dry spells.', effort: 'moderate', cost: '$$', delivery: 'gs_install', mutate: function (r) { r.soil_water.has_rainwater_system = true; } });
    if (!sw.has_swale) add({ id: 'swale', pillar: 'soil_water', label: 'Cut a swale on contour', detail: 'Slows and infiltrates runoff along the slope.', effort: 'moderate', cost: '$$', delivery: 'gs_install', mutate: function (r) { r.soil_water.has_swale = true; } });

    /* --- Habitat --- */
    var ndNext = nextThreshold(h.habitat_nodes || 0, [1, 3, 5]);
    if (ndNext !== null) add({ id: 'nodes_to_' + ndNext, pillar: 'habitat', label: 'Establish habitat node ' + ndNext, detail: 'Discrete refuges (log-and-rock piles, dense thickets) that concentrate shelter.', effort: 'quick', cost: '$', delivery: 'gs_install', mutate: function (r) { r.habitat.habitat_nodes = ndNext; } });
    if (!h.has_embedded_logs) add({ id: 'logs', pillar: 'habitat', label: 'Embed habitat logs', detail: 'Partly-buried logs for beetles, skinks and fungi.', effort: 'quick', cost: '$', delivery: 'diy', mutate: function (r) { r.habitat.has_embedded_logs = true; } });
    if (!h.has_water_feature) add({ id: 'water_feature', pillar: 'habitat', label: 'Add a small water feature', detail: 'Even a bowl or bath lifts fauna use quickly.', effort: 'quick', cost: '$', delivery: 'diy', mutate: function (r) { r.habitat.has_water_feature = true; } });
    if (!h.has_rock_refuges) add({ id: 'rocks', pillar: 'habitat', label: 'Build rock refuges', detail: 'Rock stacks give reptiles thermal shelter.', effort: 'quick', cost: '$', delivery: 'diy', mutate: function (r) { r.habitat.has_rock_refuges = true; } });
    if (!h.has_nest_boxes) add({ id: 'nest_boxes', pillar: 'habitat', label: 'Install nest boxes', detail: 'Species-matched boxes offset the shortage of old hollows.', effort: 'quick', cost: '$', delivery: 'plants_of_place', mutate: function (r) { r.habitat.has_nest_boxes = true; } });
    var faunaVerified = (h.fauna_sightings || []).filter(function (s) { return s.verified; }).length;
    if (faunaVerified < 1) add({ id: 'fauna_first', pillar: 'habitat', label: 'Log your first verified fauna sighting', detail: 'A dated, verified sighting activates the habitat fauna weighting.', effort: 'quick', cost: '—', delivery: 'diy', mutate: function (r) { r.habitat.fauna_sightings = (r.habitat.fauna_sightings || []).concat([{ verified: true }]); } });
    ...
    if (!c.corridor_node_confirmed && inCorridor) add({ id: 'corridor_node', pillar: 'connectivity', label: 'Confirm corridor-node status', detail: 'A verification visit confirms the garden as a stepping-stone in the local corridor.', effort: 'moderate', cost: '$$', delivery: 'verification', mutate: function (r) { r.connectivity.corridor_node_confirmed = true; } });
    var adjNext = nextThreshold(adj, [1, 2, 3]);
    if (adjNext !== null) add({ id: 'adjacent_' + adjNext, pillar: 'connectivity', label: (adj < 1 ? 'Register a neighbouring garden' : 'Register adjacent garden ' + adjNext), detail: 'Each registered neighbour strengthens the cluster and everyone’s connectivity score.', effort: 'moderate', cost: '—', delivery: 'diy', mutate: function (r) { var arr = (r.connectivity.adjacent_registered_gardens || []).slice(); arr.push({ verified: true }); r.connectivity.adjacent_registered_gardens = arr; } });

    /* --- Evidence & Verification --- */
    if (!e.has_species_list) add({ id: 'species_list', pillar: 'evidence', label: 'Publish a species list', detail: 'Document what’s planted — the backbone of the record.', effort: 'quick', cost: '—', delivery: 'diy', mutate: function (r) { r.evidence.has_species_list = true; } });
    if (!e.has_photos) add({ id: 'photos', pillar: 'evidence', label: 'Add dated photos', detail: 'A simple photo series evidences change over time.', effort: 'quick', cost: '—', delivery: 'diy', mutate: function (r) { r.evidence.has_photos = true; } });
    if (!e.has_field_notes) add({ id: 'field_notes', pillar: 'evidence', label: 'Keep field notes', detail: 'Short observations build the garden’s story and credibility.', effort: 'quick', cost: '—', delivery: 'diy', mutate: function (r) { r.evidence.has_field_notes = true; } });
    if (!e.has_fauna_record) add({ id: 'fauna_record', pillar: 'evidence', label: 'Start a fauna record', detail: 'Log birds, frogs and insects as you notice them.', effort: 'quick', cost: '—', delivery: 'diy', mutate: function (r) { r.evidence.has_fauna_record = true; } });
    if (!e.has_professional_assessment) add({ id: 'assessment', pillar: 'evidence', label: 'Book a professional assessment', detail: 'A G&S ecological assessment adds rigour and unlocks higher verification.', effort: 'moderate', cost: '$$', delivery: 'verification', mutate: function (r) { r.evidence.has_professional_assessment = true; } });
    ...
    if (vlNext) add({ id: 'verify_' + vlNext, pillar: 'evidence', label: 'Upgrade verification level', detail: 'Move from ' + vl.replace(/_/g, ' ') + ' to ' + vlNext.replace(/_/g, ' ') + '.', effort: 'moderate', cost: '$$', delivery: 'verification', mutate: function (r) { r.evidence.verification_level = vlNext; } });
  ...
  function buildOpportunities(record, opts) {
    ...
    raw.forEach(function (o) {
      ...
      opps.push({
        ...
        group: groupOf(o.pillar, o.delivery),
        action: o.label, detail: o.detail, points: points,
        effort: o.effort, cost: o.cost,
        delivery: o.delivery, deliveryLabel: DELIVERY_LABEL[o.delivery] || o.delivery,
  ...
  function renderOpportunities(result, mode) {
    ...
    if (mode === 'steward') { // public profile — ecological first, no pricing
      var lead = (ecological.length ? ecological : top).slice(0, 3).map(function (o) {
        return '<div class="opp-card"><div class="opp-action">' + esc(o.action) + '</div>' +
          '<div class="opp-meta">+' + o.points + ' pts · ' + esc(o.deliveryLabel) + '</div>' +
        ...
        lead += '<div class="opp-record-h">Strengthen your record</div>' +
          record.slice(0, 2).map(function (o) {
            return '<div class="opp-card opp-card--record"><div class="opp-action">' + esc(o.action) + '</div>' +
              '<div class="opp-meta">+' + o.points + ' pts · ' + esc(o.deliveryLabel) + '</div></div>';
          }).join('');
      }
      return lead;
    }
    ...
    var card = function (o) {
      return '<div class="opp-card"><div class="opp-action">' + esc(o.action) + '</div>' +
        '<div class="opp-meta">+' + o.points + ' pts · ' + esc(o.pillarLabel) + ' · ' + o.effort + ' · ' + o.cost + ' · ' + esc(o.deliveryLabel) + (o.season ? ' · ' + esc(o.season) : '') + '</div>' +
        '<div class="opp-detail">' + esc(o.detail) + '</div></div>';
    };
